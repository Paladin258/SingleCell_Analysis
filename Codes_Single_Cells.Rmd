...
# Access the meta.data slot of the Seurat object
meta_data <- SC_LRRC15_Stroma_clean@meta.data
# Create a new column 'Unique_ID' by combining 'group_time' and 'Hash_assignment'
meta_data$Unique_ID <- paste(meta_data$group_time, meta_data$Hash_assignment, sep = "_")
# Assign the updated meta.data back to the Seurat object
SC_LRRC15_Stroma_clean@meta.data <- meta_data

stroma_sce <- as.SingleCellExperiment(SC_LRRC15_Stroma_clean)
milo.stroma <- Milo(stroma_sce)
# Build neighborhood
milo.stroma <- buildGraph(milo.stroma, k=20, d=30)
milo.stroma <- makeNhoods(milo.stroma, k=20, d=30, refined=TRUE, prop=0.2)
...
da_results <- testNhoods(milo.stroma, design = ~ 0 + group_time, design.df = design.stroma, 
                         model.contrasts = contrast.1,fdr.weighting="graph-overlap", norm.method="TMM")
da_results <- annotateNhoods(milo.stroma, da_results, coldata_col = "cell_labels")
# Plot 
milo.stroma <- buildNhoodGraph(milo.stroma)

plotUMAP(milo.stroma, colour_by="cell_labels") + plotNhoodGraphDA(milo.stroma, da_results, alpha=0.05) +
  plot_layout(guides="collect") 


plotDAbeeswarm(groupNhoods(milo.stroma, da_results, max.lfc.delta = 0.5), group.by = "cell_labels")


# Assuming you have a Seurat object with UMAP already calculated, transfer the coordinates to milo.stroma
umap_coordinates <- Embeddings(SC_LRRC15_Stroma_clean, reduction = "umap")
...
library(scran)
# Extract the indices of cells labeled as endothelial cells
sce_subset <- subset(SC_LRRC15_Stroma_clean, subset = SingleR.labels  %in% c("Endothelial cells", "Fibroblasts"))
sce_subset<- as.SingleCellExperiment(sce_subset)
# Create a new combined grouping variable
sce_subset$SingleR.labels <- gsub("Endothelial cells", "Endothelial_cells", sce_subset$SingleR.labels)

# Create a new combined grouping variable
sce_subset <- sce_subset[rowSums(counts(sce_subset)) > 10,]

# Create the info DataFrame with mice group, cell clusters, and treatment group
info <- DataFrame(
  mice_group = sce_subset$Hash_assignment,         # Mice group metadata
  cluster = sce_subset$SingleR.labels,
  Treatment = sce_subset$group_time)        # Cell cluster labels    
...
design <- model.matrix(~ 0 + Treatment, data = pseudo@colData)
# Define the contrasts for D7, D14, and D21
contrast <- makeContrasts(
  LRRC15_vs_Control_D7 = TreatmentLRRC15.LT_D7 - TreatmentControl_D7,
  LRRC15_vs_Control_D14 = TreatmentLRRC15.LT_D14 - TreatmentControl_D14,
  LRRC15_vs_Control_D21 = TreatmentLRRC15.LT_D21 - TreatmentControl_D21,
  levels = design
)

# Loop over the contrast matrix columns
results_list <- lapply(colnames(contrast), function(contrast_name) {
  
  # Extract the current contrast vector
  current_contrast <- contrast[, contrast_name]
  
  # Apply pseudoBulkDGE for the current contrast
  result <- pseudoBulkDGE(pseudo, 
                          label = pseudo$cluster,  
                          design = ~ Treatment,  
                          contrast = current_contrast,
                          method = "edgeR")
  
  # Store the result along with the contrast name
  return(list(contrast = contrast_name, result = result))
})

# Access the results
 names(results_list) <- colnames(contrast)


## replace DE table
df2_D21 <- as.data.frame(results_list[["LRRC15_vs_Control_D21"]][["result"]]@listData[["Endothelial_cells"]]@listData)
df2_D21$name <- row.names(pseudo)
df_D7 <- df_D7 %>%
    mutate(combined_score = sign(logFC) * -log10(PValue))
## Filter top genes
top_genes <- df_DE  %>%
  filter(PValue < 0.01 ) %>%  # Filter significant genes
  arrange(PValue) %>%            # Sort by p-value
  slice_head(n = 50)  
## Volcano plot

ggplot(df_DE, aes(x = logFC, y = -log10(PValue))) +
  geom_point(aes(color = PValue < 0.01), size = 0.5) +
  geom_text_repel(data = top_genes, aes(label = genes), size = 2.5, max.overlaps = 100) +
  theme_minimal() +
  ggtitle("Day 14 Treatment vs Control")


# Merge df1 and df2 by the common column 'ID'
merged_df <- merge(df_D7[,c(7,6)], df_D14[,c(7,6)], by = "name", all = TRUE)

# Merge the result with df3 by the common column 'ID'
merged_df <- merge(merged_df, df_D21[,c(7,6)], by = "name", all = TRUE)

# Calculate the row-wise mean of Value1, Value2, and Value3, ignoring NA values
merged_df$Average <- rowMeans(merged_df[, c("combined_score.x", "combined_score.y","combined_score")], 
                              na.rm = TRUE)
